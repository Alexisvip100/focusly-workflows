...
async def main():
    async with async_session_local() as db:
        ...
        result = await db.execute(
            select(Task).where(Task.userId == user.id, Task.deletedAt == None)
        )
        ...
        for nt in new_tasks:
            print(
                f"  - {nt['title']} (priority={nt['priorityValue']}, deadline={nt['deadline']})"
            )
        ...
        for st in res["scheduledTasks"]:
            ...
            for wb in st["workBlocks"]:
                print(
                    f"    WorkBlock: start={wb['start']}, end={wb['end']}, duration={wb['duration']}"
                )

        print(f"Unscheduled Tasks: {len(res['unscheduledTasks'])}")
        for ut in res["unscheduledTasks"]:
            print(
                f"  Task {ut['taskId']}: reason={ut['reason']}, action={ut['suggestedAction']}"
            )
