...
class DummyInfo:
    def __init__(self, db, user_id):
        self.context = {"db": db, "user_id": user_id}
...
async def main():
    async with async_session_local() as db:
        ...
        for t in tasks:
            print(
                f"Task: {t.title}, estimated_start_date: {t.estimated_start_date}, deadline: {t.deadline}"
            )
        ...
        try:
            tasks_filtered = await query_resolver.get_tasks_by_user(
                info, user.id, filters=filters
            )
