...
class WorkspacesRepository:
    ...
    async def get_by_id(self, workspace_id: str) -> Workspace | None:
        ...
        if workspace:
            await cache.set(
                f"workspace:id:{workspace.id}", serialize_workspace(workspace)
            )
        return workspace
    ...
    async def get_all_by_user(self, user_id: str) -> list[Workspace]:
        ...
        await cache.set(
            f"workspaces:user:{user_id}", [serialize_workspace(w) for w in workspaces]
        )
        return workspaces
    ...
    async def find_all(
        ...
        if search:
            ...
            filter_cond = (Workspace.title.ilike(search_pattern)) | (
    ...
    async def release_taskId_for_other_workspaces(
        self, task_id: str, exclude_workspace_id: str, now: datetime
    ) -> None:
...
class ProjectGroupsRepository:
    ...
    async def delete_workspaces_by_group_id(self, group_id: str) -> None:
        ...
        await self.db.execute(delete(Workspace).where(Workspace.groupId == group_id))
        await self.db.flush()
        await cache.delete_pattern("workspaces:user:*")
        await cache.delete_pattern("workspace:id:*")
