...
from .time_blocks_mapper import time_block_to_dict, parse_time_block_dt
...
class TimeBlocksService:
    ...
    def _parse_dt(self, val: Any) -> datetime | None:
        return parse_time_block_dt(val)
    ...
    async def find_all(self) -> list[dict[str, Any]]:
        ...
        return [time_block_to_dict(tb) for tb in blocks]
        ...
        for key, value in update_data.items():
            ...
            if hasattr(tb, key):
                if key in ["startTime", "endTime"]:
                    setattr(tb, key, parse_time_block_dt(value))
                else:
