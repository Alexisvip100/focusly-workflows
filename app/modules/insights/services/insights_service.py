...
class InsightsService:
    ...
    async def getInsights(
        ...
        if self.tasks_service:
            ...
            all_tasks = (
                tasks_res.get("items", []) if isinstance(tasks_res, dict) else tasks_res
            )
        else:
            tasks_list = await TasksRepository(self.db).get_active_non_google_tasks(
                user_id
            )
        ...
        if self.focus_sessions_service:
            ...
            all_focus_sessions = await FocusSessionsRepository(self.db).get_all_by_user(
                user_id
            )
        ...
        for t in all_tasks:
            updated_str = (
                t.get("completedAt") or t.get("updatedAt") or t.get("createdAt")
            )
            task_date = (
        ...
        for s in all_focus_sessions:
            ...
            if isinstance(s_dt, str):
                s_dt = datetime.fromisoformat(s_dt.replace("Z", "+00:00")).replace(
                    tzinfo=None
                )
        ...
        golden_window = self.calculate_golden_window(
            filtered_sessions, work_hours_config
        )
        ...
        productivity_trends = self.calculate_productivity_trends(
            all_tasks, filtered_sessions, filter_type, now
        )
        ...
        heatmap_data, heatmap_labels, heatmap_cells = self.calculate_activity_map(
            all_tasks, filter_type, timezone_offset_minutes, now
        )
    ...
    def calculate_activity_map(
        ...
        if filter_type == "Daily":
            return self._build_daily_activity_map(
                completed_tasks, now, timezone_offset_minutes
            )
        if filter_type == "Monthly":
            return self._build_monthly_activity_map(
                completed_tasks, now, timezone_offset_minutes
            )
        if filter_type == "Yearly":
            return self._build_yearly_activity_map(
                completed_tasks, now, timezone_offset_minutes
            )
        return self._build_weekly_activity_map(
            completed_tasks, now, timezone_offset_minutes
        )
    ...
    def _build_daily_activity_map(
        ...
        for task in completed_tasks:
            ...
            if completed_at:
                ...
                local_completed_at = completed_at - timedelta(
    ...
    def _build_monthly_activity_map(
        ...
        bucket: dict[str, list[dict[str, Any]]] = {
            d.isoformat(): [] for d in month_days
        }
    ...
    def build_daily_trends(
        self, tasks: list[dict[str, Any]], sessions: list[FocusSession], now: datetime
    ) -> list[dict[str, Any]]:
        today = now.date()
        trends = []
