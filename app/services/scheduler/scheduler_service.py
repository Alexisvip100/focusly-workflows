...
from app.models.models import Task, TimeBlock, User
...
class SchedulerService:
    ...
    def _build_hard_constraints(
        self, external_events: list[dict[str, Any]], meetings: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
        constraints = []
    ...
    def _filter_tasks_needing_scheduling(
    ...
    def _find_available_slots(
        ...
        while current_ptr < end_time:
            ...
            if self._is_overlapping_with_work_blocks(
                current_ptr, check_end, existing_work_blocks
            ):
                current_ptr += timedelta(minutes=slot_size)
                continue
            ...
            while (
                self._is_within_working_hours(slot_end, constraints)
                and not self._is_overlapping(
                    slot_end, slot_end + timedelta(minutes=slot_size), hard_constraints
                )
                and not self._is_overlapping_with_work_blocks(
                    slot_end,
    ...
    def _is_overlapping_with_work_blocks(
    ...
    def _create_work_block(
        ...
        return {
            ...
            "blockType": "focus",
            "isGenerated": True,
            "schedulingScore": self._calculate_scheduling_score(
    ...
    async def run_scheduling_pipeline(
        ...
        print(
            f"[SCHEDULER] Running Motion-style scheduler pipeline for user: {user_id}"
        )
        ...
        work_hours_config = settings_dict.get(
            "workHoursConfig",
            {
