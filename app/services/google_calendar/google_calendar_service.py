...
class GoogleCalendarService:
    ...
    async def sync_calendar(self, user_id: str) -> None:
        ...
        delete_result = await self.db.execute(
            delete(Task).where(Task.userId == user_id, Task.source == "google")
        )
        deleted_count = delete_result.rowcount
        if deleted_count > 0:
            await self.db.commit()
            print(
                f"[SYNC] Deleted {deleted_count} legacy Google tasks from database for user {user_id}"
            )
            has_changes = True
        ...
        if sync_token and not existing_google_tasks:
            print(
                f"Sync token exists but 0 GoogleTasks found in DB for user {user_id}. Clearing sync token to force a full sync."
            )
            sync_token = None
            ...
            try:
                while True:
                    ...
                    if res.status_code == 410:
                        print(
                            f"Sync token expired for user {user_id}. Retrying with full sync."
                        )
                        user.googleCalendarSyncToken = None
                        await self.db.commit()
                        sync_token = None
                ...
                print(
                    f"Fetched {len(items_to_process)} events/changes to process for user {user_id}"
                )
                for item in items_to_process:
                    ...
                    if item.get("status") == "cancelled":
                        print(
                            f"Sync Cancelled/Deleted Event: {event_id} for user {user_id}"
                        )
                        # Find existing task
                        ...
                        if not existing_task:
                            print(
                                f"[SYNC] Skipping creation of task for google event {event_id} because it only exists in Google Calendar."
                            )
                            continue
                ...
                if has_changes:
                    print(
                        f"Sync Calendar detected changes. Running scheduler for user: {user_id}"
                    )
                    ...
                    print(
                        f"Sync Calendar detected no actual changes for user: {user_id}. Skipping scheduler."
                    )
    ...
    async def watch_calendar(self, user_id: str) -> None:
        try:
            ...
            if expiration and expiration - now > 24 * 60 * 60 * 1000:
                print(
                    f"Watch channel for user {user_id} is still valid. Skipping setup."
                )
                return
            ...
            if not webhook_url:
                print(
                    f"WEBHOOK_URL not set in settings. Cannot establish watch channel for user {user_id}."
                )
                return
            ...
            async with httpx.AsyncClient() as client:
                ...
                if res.status_code != 200:
                    print(
                        f"Failed to register watch channel. Status: {res.status_code}. Body: {res.text}"
                    )
                    return
            ...
            print(
                f"Watch channel registered for user {user_id}. Expires: {datetime.fromtimestamp(user.googleChannelExpiration / 1000).isoformat()}"
            )
