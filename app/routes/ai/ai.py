...
async def chat_endpoint(
    ...
    conv_result = await db.execute(
        select(Conversation).filter(Conversation.userId == current_user_id)
    )
    ...
    selected_model = body.model or (
        "gemini-2.5-pro" if complexity == "complex" else "gemini-2.5-flash-lite"
    )
    ...
    gemini_contents = [{"role": "user", "parts": [{"text": latest_user_message}]}]
