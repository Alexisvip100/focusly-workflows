...
async def handle_google_webhook(
    ...
    print(
        f"Google Webhook received for user: {x_goog_channel_token}, state: {x_goog_resource_state}, channelId: {x_goog_channel_id}"
    )
        ...
        async def run_sync_bg():
            ...
            async with async_session_local() as local_db:
                ...
                try:
                    ...
                    print(
                        f"[WS] Emitted schedule_updated to user {userId} after Google webhook sync"
                    )
                except Exception as err:
                    print(
                        f"Failed to execute incremental sync for user {userId} via webhook: {err}"
                    )
